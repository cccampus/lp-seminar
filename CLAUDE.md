...
- 2026-06-25: **2026/7/26(第4回)を全面削除**（7/8を最後に一旦セミナー終了の方針）。日程追加フローの逆を全箇所適用: seminar-sessions.ts / HeroMetaA(7/8単独) / DetailB(単独カードを `grid-cols-1 max-w-md mx-auto` で中央寄せ) / FinalCTA / FinalCTAStart / NextSession(死にコード) / dashboard(SESSION_LABELS+KPI+otherCount filter) / cron reminders / zoom-setup。**デプロイ罠**: `vercel` セッションが個人(kiyos)に戻ってて cccampus-projects が触れず「Could not retrieve Project Settings / Not authorized」→ `git push origin main`(cccampus/lp-seminar)で auto deploy は走るが **ccc-seminar.vercel.app alias は自動昇格しない**ので、cccampus 再ログイン→`vercel ls lp-seminar --scope cccampus-projects` で最新 Ready deploy 取得→`vercel alias set <deploy> ccc-seminar.vercel.app --scope cccampus-projects` 手動必須。7/26 Zoom/env(`ZOOM_*_20260726`)は孤児として残置（無害・再開時に再利用可）
