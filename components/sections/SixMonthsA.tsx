...
export default function SixMonthsA() {
  return (
    <section
      id="six-months"
      ...
      <div className="relative z-10 flex min-h-[100svh] flex-col justify-end px-6 py-24 sm:min-h-0 sm:justify-start sm:py-32 lg:px-12">
        <div className="mx-auto w-full max-w-3xl sm:max-w-2xl lg:max-w-3xl">
          ...
          */}
          ...
          {/* Before / After — エディトリアル罫線（パネル廃止・ログラス系） */}
          <div className="mt-14 sm:mt-20 divide-y divide-cream/15 border-y border-cream/20">
            {beforeAfter.map((item, i) => (
              <div
                key={i}
                className="grid grid-cols-[auto_1fr] sm:grid-cols-[auto_minmax(0,1.1fr)_auto_minmax(0,0.9fr)] items-baseline gap-x-5 sm:gap-x-10 gap-y-1 py-7 sm:py-9"
              >
                {/* 番号 */}
                <span className="font-mono text-[11px] sm:text-xs tracking-[0.3em] text-coral/85 self-start pt-2 sm:pt-3">
                  {String(i + 1).padStart(2, "0")}
                </span>
                {/* ラベル */}
                <p
                  className="font-serif text-xl sm:text-3xl font-semibold text-cream leading-snug"
                  style={{ letterSpacing: "-0.01em" }}
                >
                  {item.label}
                </p>
                {/* Before（スマホは下段の番号列にぶら下げ） */}
                <p className="col-start-2 sm:col-start-3 font-serif text-sm sm:text-base text-cream/55 line-through decoration-coral/40 decoration-1 underline-offset-4 self-start sm:pt-1">
                  {item.before}
                </p>
                {/* After */}
                <p
                  className="col-start-2 sm:col-start-4 font-serif text-lg sm:text-2xl font-semibold text-coral self-start"
                  style={{ letterSpacing: "-0.01em" }}
                >
                  {item.after}
                </p>
